...
resource "google_secret_manager_secret_iam_binding" "secret_webhook_cloudbuild_trigger_cicd_access" {
  project = var.project_id
  secret_id = google_secret_manager_secret.secret_webhook_cloudbuild_trigger_cicd.id
  role    = "roles/secretmanager.secretAccessor"

  members = [
    "serviceAccount:${data.google_service_account.playground_ci_sa.account_id}",
    "serviceAccount:${data.google_service_account.playground_cd_sa.account_id}"
  ]
}
